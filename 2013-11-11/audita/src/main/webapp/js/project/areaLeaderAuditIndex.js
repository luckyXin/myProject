		...
		function() {
			...
			$("#audit").click(
					function() {
						...
						if ($("#legalState").val() == "1") {
							$.messager.confirm($("#title").val(),
									'确定要跳过法制科的审批？',
									function(r) {
										if (r) {
											top.openModule($("#frameId").val(), '分管领导审批-录入',
													'/project/AreaLeaderAudit/input.do', null,
													'legalState', row.id);
										}
									});
						} else {
...
function initialization() {
	...
	$('#grid').datagrid(
			{
				url : $("#coutextPath").val()
						+ '/project/AreaLeaderAudit/find.do?arrangeType='
