		...
		function() {
			...
			$("#search").click(function() {
				$("#grid").datagrid('options').queryParams = {
					...
					auditIdentity : $("#auditIdentity").val(),
			...
			$("#excelallfinishpro").click(function(){
							...
							var auditIdentity = $("#auditIdentity").val();
							var employeeName =$("#employeeId").val();
						    var url=$("#coutextPath").val()+ '/common/project/importfinishprojectexcel.do?datapreStartTime='+auditStartTime+'&datapreEndTime='+auditEndTime+'&auditidentity='+auditIdentity+'&userId='+employeeName+'&method=1';
			...
			$("#audit").click(
					function() {
						...
						top.openModule($("#frameId").val(), '项目明细展示',
								'/search/employeeAudit/input.do', null,
								'add', row.projectId);
					});
			initialization();
