		...
		function() {
			...
			$("#search").click(function() {
				$("#grid").datagrid('options').queryParams = {
					...
					acceptStartTime : $("#auditStartTime").datebox("getValue"),
					acceptEndTime : $("#auditEndTime").datebox("getValue")
			...
			$("#audit").click(
					function() {
						...
						top.openModule($("#frameId").val(), '项目明细展示',
								'/search/mySelfNoCompleteProject/input.do', null,
								'add', row.projectId);
					});
...
function initialization() {
	...
	$('#grid').datagrid(
			{
				url : $("#coutextPath").val()
						+ '/search/mySelfNoCompleteProject/find.do',
				columns : [ [ {
					field : 'projectName',
					title : '项目名称',
					width : fillsize(0.2),
