...
<html>
	<head>
		<title>个人跟踪审计项目查询</title>
		<jsp:include page="../common/include.jsp"></jsp:include>
		<script type="text/javascript"
			src="<%=request.getContextPath()%>/js/commonFunction.js"></script>
		<script type="text/javascript"
			src="<%=request.getContextPath()%>/js/search/OneTractAuditProjectSearchIndex.js"></script>
		<style type="text/css">
body {
	margin-left: 0px;
	margin-top: 0px;
	...
	<body>
		<div id="mainDiv" style="width: 100%; margin: 5px;">
					...
					<div id="center" style="margin-top: 10px;" align="center">
						<table class="form" cellspacing="0" width="99%">
							<tbody>
								...
								<tr>
								    <td align="right" width="20%">清标状态：</td>
								    <td>
								      <select id="qingBiaoState" style="width: 200px;" class="easyui-combobox" >
										<option value="0">未清标</option>
										<option value="1">已清标</option>
										<option value="2" selected="selected">全部</option>
									  </select>
								    </td>
								    <td align="right" width="20%">是否移交：</td>
									<td>
										<select id="dataTransferState" name="dataTransferState"
								...
								<td align="center" colspan="4">
										...
										<a href="javascript:void(0);" id="reset"  iconCls="icon-cancel" class="easyui-linkbutton">重置</a>
									</td>
								</tr>
							</tbody>
						</table>
			...
			<div id="p2" class="easyui-panel" style="background: #fafafa;"
				collapsible="true" title="项目信息" iconCls="icon-tip">
					...
					<c:forEach items="${mapFunction}" var="con">
						...
						<c:if test="${con=='update.do'}">
							<a href="javascript:void(0);" id="update" iconCls="icon-edit"
								class="easyui-linkbutton">明细展示</a>
						</c:if>
						
					</c:forEach>
					<a href="javascript:void(0);" id="daoexceltz" iconCls="icon-print"
								class="easyui-linkbutton">跟踪审计台账导出</a>
					<a href="javascript:void(0);" id="templateimport" iconCls="icon-add"
								class="easyui-linkbutton">模板导入</a>			
				</div>
				<table id="grid"></table>
			</div>
