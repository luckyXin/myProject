...
<html>
	<head>
		<title>分管领导安排项目确认</title>
		<jsp:include page="../common/include.jsp"></jsp:include>
		<script type="text/javascript"
			src="<%=request.getContextPath()%>/js/commonFunction.js"></script>
		<script type="text/javascript"
			src="<%=request.getContextPath()%>/js/project/areaLeaderArrangeInfoAffirmIndex.js"></script>
		<style type="text/css">
body {
	margin-left: 0px;
	margin-top: 0px;
						...
						<c:forEach items="${commonFunction}" var="con">
							...
							<c:if test="${con=='update.do'}">
								<a href="javascript:void(0);" id="update"
									iconCls="icon-mini-edit" class="easyui-linkbutton">查看</a>
							</c:if>
