$(document).ready(
		function() {
			...
			$("#intermediaryName").click(function() {
				openshowdiv(4, "中介机构", 580, 350,270, null);
			});
			$("#search").click(function() {
				$("#grid").datagrid('options').queryParams = {
					...
					intermediary : $("#intermediaryId").val(),
					construction : $("#constructName").val(),
					buildYear : $("#buildYear").datebox("getValue"),
			...
			$("#excelallfinishpro").click(function(){
				...
				var intermediary=$("#intermediaryId").val();
			    var construction =$("#constructName").val();
				var projectOwner= $("#ownerId").val();
			    var url=$("#coutextPath").val()+ '/common/project/importfinishprojectexcel.do?datapreStartTime='+auditStartTime+'&datapreEndTime='+auditEndTime+'&intermediaryId='+intermediary+'&constructId='+construction+'&projectOwner='+projectOwner+'&method=1';
			...
			$("#audit").click(
					function() {
						...
						top.openModule($("#frameId").val(), '项目明细展示',
								'/search/relevantProjectSearch/input.do', null,
								'add', row.projectId);
					});
			initialization();
...
function initialization() {
	...
	$('#grid').datagrid(
			{
				url : $("#coutextPath").val()
						+ '/search/relevantProjectSearch/find.do',
				frozenColumns : [ [ {
					field : 'datapreno',
					title : '送审编号',
					width : fillsize(0.08),
