...
<html>
	<head>
		...
		<script type="text/javascript"
			src="<%=request.getContextPath()%>/js/search/noCompleteProjectSearch.js"></script>
		<style type="text/css">
body {
	margin-left: 0px;
	margin-top: 0px;
		...
		<div id="mainDiv" style="width: 100%; margin: 5px;">
			<div id="subDiv" style="margin-bottom: 5px;">
							...
							<table class="form" cellspacing="0" width="99%">
								<tbody>
									...
									<tr>
									   ...
									   <td align="right" width="20%">是否暂停：</td>
									   <td>
									       <select id="isStop" name="isStop" style="width: 200px;">
									            <option value="0">全部</option>
									            <option value="1">已暂停</option>
									            <option value="2">未暂停</option>
									       </select>
									   </td>
									</tr>
									<tr>
				...
				<div id="p2" class="easyui-panel" style="background: #fafafa;"
					collapsible="true" title="未完成项目列表" iconCls="icon-tip">
					<div id="button-bar" style="margin: 5px;">
						<c:forEach items="${mapFunction}" var="con">
							...
							<c:if test="${con=='update.do'}">
							    ...
							    <a href="javascript:void(0);" id="openStopProject"
									iconCls="icon-mini-add" class="easyui-linkbutton">重新开启</a>
							</c:if>
							<c:if test="${con=='find.do'}">
								<input type="hidden" id="find" value="find">
							</c:if>
						</c:forEach>
						<a href="javascript:void(0);" id="excelallnocompro"
