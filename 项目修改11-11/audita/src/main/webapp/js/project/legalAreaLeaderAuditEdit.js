...
$(document).ready(
	...
	$("#sj4").text($("#sj4").text().substr(0,10))
			...
			$("#save").click(
					function() {
						var url = $("#coutextPath").val()+ '/project/legalAreaLeaderAudit/audit.do';
						if ($("#auditId").val() != "") {
							url = $("#coutextPath").val()+ '/project/legalAreaLeaderAudit/update.do';
						}
						$("#form").ajaxSubmit(
								{
									url : url,
...
function loadlegalfile() {
	var height = document.body.clientHeight;
    var width = $(document.body).width() ;
	$('#legalmyfile').datagrid({   
	    ...
	    //设置分页控件   
	    var p = $('#legalmyfile').datagrid('getPager');   
	    $(p).pagination({   
