...
$(document).ready(
	...
	$("#sj5").text($("#sj5").text().substr(0,10))
	$("#sj6").text($("#sj6").text().substr(0,10))
	$("#sj7").text($("#sj7").text().substr(0,10))
			...
			$("#save").click(
					function() {
						var url = $("#coutextPath").val()+ '/project/legalSectionChiefAudit/audit.do';
						if ($("#auditId").val() != "") {
							url = $("#coutextPath").val()+ '/project/legalSectionChiefAudit/update.do';
						}
						$("#form").ajaxSubmit(
								{
									url : url,
...
function loadlegalsecfile() {
	var height = document.body.clientHeight;
    var width = $(document.body).width() ;
	$('#legalsecmyfile').datagrid({   
	    ...
	    //设置分页控件   
	    var p = $('#legalsecmyfile').datagrid('getPager');   
	    $(p).pagination({   
