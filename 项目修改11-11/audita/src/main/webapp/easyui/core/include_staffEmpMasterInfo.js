 ...
 * 主审人员基本信息
 * 使用是将该JS引入
 * 页面上必须包含id有staffEmpGov_info_id(主审人员基本信息)
 * 页面上必须包含变量 var isTrue = false/true;(是否可以编辑)
 */


$(function() {
	$("#staffEmpMaster_info_id").combobox({
		valueField : 'code',
		...
		onShowPanel : function() {
			$("#staffEmpMaster_info_id").combobox("hidePanel");
			aaa.window("open");
		}
	});
	...
	aaa.find("#adasdasdsadsdsdsads").datagrid(
			{
				...
				onDblClickRow : function(rowIndex, rowData) {
					$("#staffEmpMaster_info_id").combobox("select", rowData.code);
					aaa.window("close");
				},
				onLoadSuccess : function() {
					aaa.find("#adasdasdsadsdsdsads").datagrid("selectRecord",
							$("#staffEmpMaster_info_id").combobox("getValue"));
				}
			});
});