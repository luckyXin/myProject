...
<html>
	<head>
		...
		<script type="text/javascript"
			src="<%=request.getContextPath()%>/js/project/tractProjectQingBiao.js"></script>
...
.cancel a {
	...
	<body>
		<form id="form" method="post">
				...
				style="background: #fafafa; height: 320px; margin-bottom: 5px;"
			...
			<div id="p3" class="easyui-panel"
				...
				<div style="margin: 1px;">
				  <input type="hidden" id="htaudit" value="${htaudit}">
				    <input type="hidden" id="suopei" value="${suopei}">
				    <input type="hidden" id="policy" value="${policy}">
				    <input type="hidden" id="dataeng" value="${dataeng}">
				    <input type="hidden" id="other" value="${other}">
				    
				    
				        <c:if test="${not empty htaudit}">
				           <a href="javascript:void(0);" id="addBiaoDuan" class="easyui-linkbutton" iconCls="icon-add">录入</a>
				           <a href="javascript:void(0);" id="addBiaoDuanedit" class="easyui-linkbutton" iconCls="icon-edit">编辑</a>
				        </c:if>
				        <c:if test="${not empty suopei}">
				           <a href="javascript:void(0);" id="addBiaoDuan" class="easyui-linkbutton" iconCls="icon-add">录入</a>
				        ...
				        <c:if test="${not empty qiaobiao}">
				         <a href="javascript:void(0);" id="addBiaoDuan" class="easyui-linkbutton" iconCls="icon-edit">录入清标</a>
				        </c:if>
